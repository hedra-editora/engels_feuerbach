...
\addcontentsline{toc}{chapter}{Capítulo \versal{I}}
\section{I}
...
uma boa geração para trás, mas que já se tornou tão estranho %(\emph{fremd})
...
ambas parecem! Os franceses em luta aberta com todo o saber %(\emph{Wissenschaft})
...
chamava Heinrich
Heine.\endnote{Referência ao livro de Heine, \emph{Zur
  ...
  \emph{jovens alemães}, movimento literário %(\emph{Junge Deutschland})
...
em Hegel,
de modo nenhum tudo aquilo que existe é também automaticamente efetivo. Para ele, o atributo da realidade %(\emph{ohne weiteres auch wirklich})
...
fiscal''\endnote{\emph{Enzyklopädie der philosophischen Wissenschaften \versal{I},} § 142,
  ...
  que ilustra essa situação mencionada por Engels é justamente o ``plano, ou assim chamada ideia, de certa instituição fiscal'', %(\emph{Plan})
  que pode ser ``em si boa ou adequada aos fins, mas %(\emph{zweckmäßig})
...
como algo efetivo. O que, porém, é necessário, comprova"-se, em última
...
perverso, mas apesar da sua perversidade continua a % (\emph{schlecht})
...
de Hegel,
nesta outra: tudo o que existe é digno de perecer. %(\emph{zugrunde geht})
...
que se eleva de estágios inferiores do %(\emph{Wissenschaft})
...
conhecimento, também a história não pode encontrar um desfecho pleno %(\emph{vollendenden})
em um estado ideal perfeito de %(\emph{vollkommen})
...
no curso de desenvolvimento sem fim da sociedade humana, %(\emph{endlos})
...
é relativo, seu caráter revolucionário é %\emph{(Anschauungsweise}) 
absoluto --- é o único elemento absoluto que ela pode admitir como valido.\est\
...
``exterioriza''-se, isto é, transforma"-se na %(\emph{entäussert)}
...
bastante dócil. A forma específica dessa conclusão %(\emph{zahmen})
...
de todas as contradições. Mas, se todas as %(\emph{Überwindung}) 
...
charlatão'' dominou do modo mais exclusivo possível e %(\emph{Hegelei})
...
intuições tendenciosas da prática; e, na %(\emph{Parteianschauungen})
...
da religião tradicional e do Estado existente. E se %(\emph{Vernichtung}) 
nos \emph{Anais alemães}
os fins últimos práticos ainda se mostravam %(\emph{Deutsche Jahrbücher}) 
...
por meio da formação mitológica\est\ inconscientemente %(\emph{Gemeinde}) 
...
``consciência"-de"-si'' seria a potência decisivamente %(\emph{Macht}) 
...
de Hegel,
apenas a ``exteriorização alienante'' da ideia %(\emph{Entäusserung})
...
imaginação, foi dissolvida. É preciso ter vivido o %(\emph{Einbildung})
...
proletariado pela transformação econômica da %(\emph{Umgestaltung}) 
...
amoroso, do tipo do
senhor Karl
Grün.\endnote{Marx e Engels criticam o ``socialismo
...
rodeios. Ela tinha de ser ``superada'' no seu sentido %(aufgehoben) 
...
\addcontentsline{toc}{chapter}{Capítulo \versal{II}}
\section{II}
...
constituição corporal e incitados por aparições no sonho,\endnote{Ainda hoje, entre selvagens e bárbaros inferiores, é a representação
...
religiosa de consolação, mas a aporia proveniente %(\emph{Verlegeneheit}) 
...
efetivo uma imagem especular correta da realidade %(\emph{Spiegelbild}) 
...
Ao lado desses há, porém, ainda uma série de outros filósofos que
...
da filosofia, é a \emph{praxis}, a saber, o %(\emph{Schrullen}) 
...
kantiana é tentada na Alemanha pelos neokantianos\endnote{O
  ...
  humano'' (Ollig, Hans"-Ludwig, %(\emph{angeborene Gattungsorganisation}) 
...
Inglaterra (onde nunca morreu) pelos agnósticos,\endnote{A
  ...
  ciências da natureza no século \versal{XIX}, pressuposto da ``querela do
  materialismo'' diante do transcendental, %(\emph{Materialismusstreit})
...
progresso da ciência da natureza \textbar{} e da indústria \textbar{}. Nos materialistas
...
Por isso, é concebível que Starcke, na sua caracterização
de Feuerbach,
...
produto do espírito, mas o próprio espírito é apenas %(\emph{Erzeugnis}) 
o produto supremo da matéria. Isso é, naturalmente, %(\emph{Produkt}) 
...
explicitou em um estágio histórico determinado, \textbar{} no século \versal{XVIII} \textbar{}.
...
A biologia ainda %(\emph{phlogistischen})
...
da natureza era, portanto, inevitável. \textbar{} %(\emph{unhistorische}) 
É possível tão pouco censurar os filósofos do século \versal{XVIII} por\est\ isso,
como também é possível censurar Hegel. Para ele, a natureza, como mera
``exteriorização alienante'' da ideia, não é capaz %(\emph{Entäußerung}) 
...
simples interrupção da história por uma barbárie %(\emph{Unterbrechung}) 
...
da conexão e, assim, da ordem nesse caos de %(\emph{Herstellung}) 
...
por Darwin,
teoria da evolução. Mas como o solitário %(\emph{Entwicklungstheorie})
...
por espirituosos e ecléticos esmagadores de pulgas, %(\emph{Flohknackern})
enquanto Feuerbach,
...
das assim chamadas ciências históricas e filosóficas, %(\emph{Inbegriff}) 
...
que o impeliu a produzir pensamentos a partir da sua %(\emph{Verödung})
...
fundamentação, a infraestrutura do todo permanece, %(\emph{Unterbau}) 
...
máximo, enquanto se preocupa em através sar a ressaca moral %(\emph{Katzenjammer}) 
...
\addcontentsline{toc}{chapter}{Capítulo \versal{III}}
\section{III}
...
filosofia deve se transformar em religião. ``Os %(\emph{aufgehen in}) 
...
dos homens, cobiça e ânsia de domínio, que se % (\emph{Herrschsucht})
...
é o próprio ``mestre"-escola de Sadowa''\endnote{Possivelmente uma referência à Batalha de
...
Mais ainda. Segundo a teoria da moral
de Feuerbach,
a bolsa de valores é o templo supremo da eticidade --- % (\emph{Fondsbörse})
...
comprova"-se por meio disso ser mal\est\ calculada, como imoral, %(\emph{unsittlich})
...
Mas como era possível que a impulso violento, dado
por Feuerbach, tenha
...
\addcontentsline{toc}{chapter}{Capítulo \versal{IV} \medskip}
\section{IV}
...
consciência"-de"-si; essa consciência"-de"-si elabora a si mesmo %(\emph{sich herausarbeiten})
...
retrocessos momentâneos --- é, portanto, apenas a imitação %(\emph{Abklatsch}) 
...
cabeça como imagens derivadas de coisas efetivas, em % (\emph{Abbilder})
vez de apreender as coisas efetivas como imagens derivadas
deste ou daquele estágio do conceito absoluto. %(\emph{Abbilder}) 
...
de leis que, segundo o movimento da coisa em questão, são %(\emph{Sache})
...
de Hegel era
posta acima da cabeça, ou, antes: da % (\emph{auf den Kopf gestellt})
...
investigação partimos sempre desse ponto de vista, %(\emph{Gesichtspunkt})
...
investigação das \emph{coisas }como elementos duradores, %(\emph{Bestände}),
...
proporções transitam %{[}\emph{Maßverhältnissen} --- relações de medida{]}
...
por Darwin,
de que o elemento durador dos produtos orgânicos da %(\emph{Bestand}) 
...
singulares por uma conexão feita na cabeça do % (\emph{Ereignissen})
...
desconhecida, estabelecia"-se constitutivamente, assim, %(\emph{setzte})
...
pela adivinhação  das efetivas; uma tarefa que %(\emph{Auffindung})
...
Aqui, no entanto, a história do desenvolvimento da sociedade mostra"-se
...
repercussão da ação do homem sobre a natureza --- há %(\emph{Rückwirkung}) 
...
resultados que confirmam a regularidade no % (\emph{Gesetzmäßigkeit})
...
parte, \emph{fundamentos ideais do movimento}, %(\emph{ideelle Beweggründe})
...
regra, que os nobres são os enganados e os não %(\emph{Geprellten})
...
móveis, que é preciso investigar; mas ela não procura %(\emph{Mächte}) 
...
quebrado em pedaços as máquinas, como ainda em 1848 no Reno.\endnote{Possível alusão aos ocorridos na noite de 16 e 17 de Março de 1848 na
...
enigma. Desde a efetivação da grande indústria, %(\emph{Durchführung}) 
...
a terceira força por essa dominação. As relações se % (\emph{Kämpfer})
...
Pelo menos, na história moderna está assim demonstrado que todas as
...
reflexo, em forma vinculativa, das carências % (\emph{zusammenfassend})
...
depender das circunstancias, de modo satisfatório  ou %(\emph{gut})
perverso. %(\emph{schlecht}).

No Estado, apresenta"-se para nós a primeira potência %(\emph{Macht})
...
externos. Esse órgão é o poder do Estado. Assim que %(\emph{Gewalt}) 
...
econômicas torna"-se mais indeterminada %(\emph{Unterlage}) 
e pode se perder totalmente. Onde isso não é %(\emph{dumpfer}) 
...
O Estado, porém, uma vez que se torna um poder autônomo %(\emph{Macht}) 
...
originariamente bastante silvestre, a partir %(\emph{Waldursprunglich})
...
filosofia, política, prática do %(\emph{Jurisprudenz}){[}.{]}
...
nascia Voltaire.
A medida violenta
de Luís \versal{XIV} apenas facilitou à burguesia francesa para que pudesse fazer a sua
...
que lhe corresponde: a aristocracia rural possuidora de % (\emph{Junker})
...
Estados, os restos % (\emph{Kleinstaaterei})
...
filosofia clássica, com maior razão, o velho %(\emph{theoretisch"-rücksichtslose})
espírito teórico"-brutal:\endnote{Aqui é
  ...
  restrições  --- aquela que não olha para trás %(\emph{rücksichtlos})
  impulsionada automaticamente pela verdade da %(\emph{rück"-sicht}) 
  ...
  ``brutal'' da crítica: ``brutal tanto no sentido %(\emph{rücksichtlos}) 
  ...
  estabelecidos''. \versal{MARX}, K. Deutsche Französische Jahrbücher 1.
  Doppellieferund, Februar, 1844. In: \versal{MARX}, K.; \versal{ENGELS}, F. \emph{Werke}.
  Band 1. Berlin/\versal{DDR}: Dietz Verlag, 1976, p. 344. {[}\versal{N.\,T.}{]}} ecletismo desprovido
de pensamento, preocupação angustiada com carreiras e rendimentos
descendo até ao arrivismo  mais ordinário, tomam seu %(\emph{Strebertum})
...
contrário, quanto mais sem restrições e %(\emph{rücksichtlos}) 
